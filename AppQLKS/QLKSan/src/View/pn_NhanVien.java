 ...
 * @author 84336
 */
public class pn_NhanVien extends javax.swing.JPanel {

    /**
     * Creates new form pn_NhanVien
     */
    public pn_NhanVien() {
        initComponents();
        load();
    ...
    private void initComponents() {
        ...
        tbDichVu.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "MANV", "Tên", "Ngày sinh", "Địa chỉ", "CMND", "Lương"
            }
        ));
    ...
    private void tbDichVuMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbDichVuMouseClicked
        ...
        if(evt.getClickCount() == 2){
            fr_NhanVien.MaNV = Integer.parseInt(tbDichVu.getModel().getValueAt(tbDichVu.getSelectedRow(), 0).toString());
            fr_NhanVien form = new fr_NhanVien(this);
    ...
    public void load(){
        ArrayList<NhanVien> DSNhanVien = NhanVienController.layDSNhanVien();
        DefaultTableModel model = (DefaultTableModel) tbDichVu.getModel();
        model.setRowCount(0);
        for(NhanVien nv : DSNhanVien){
            Object[] objmodel = new Object[]{nv.getMANV(),nv.getTENNV(),nv.getNGAYSINH(),nv.getDIACHI(),nv.getCMND(),nv.getLUONG()};
            model.addRow(objmodel);
        }
    }
    
    public void loadTimKiem(String tenNV){
        ArrayList<NhanVien> DSNhanVien = NhanVienController.layDSNhanVienTheoTen(tenNV);
        DefaultTableModel model = (DefaultTableModel) tbDichVu.getModel();
        model.setRowCount(0);
        for(NhanVien nv : DSNhanVien){
            Object[] objmodel = new Object[]{nv.getMANV(),nv.getTENNV(),nv.getNGAYSINH(),nv.getDIACHI(),nv.getCMND(),nv.getLUONG()};
