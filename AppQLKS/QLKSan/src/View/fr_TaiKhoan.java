...
public class fr_TaiKhoan extends javax.swing.JFrame {
    ...
    private boolean kiemtrathongtin(){
        if(txtTenTaiKhoan.getText().equals("")){
            JOptionPane.showMessageDialog(this,"Nhập tên TK");
            return false;
        }
        return true;
    }
    ...
    private void hienthongtin(String TenTK){
        ...
        for(TaiKhoan tk : DSTK){
            ...
            cbbNhanVien.setSelectedItem(TaiKhoanController.LayTenNhanVien(tk.getMANV()));
            cbbQuyen.setSelectedItem(TaiKhoanController.Quyen(tk.getQUYEN()));
        }
    }
    ...
    private void btSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btSaveActionPerformed
        if(kiemtrathongtin()){
            if(TenTK.equals("")){
                ...
                if(!TaiKhoanController.kiemtraDB(txtTenTaiKhoan.getText()))
                {
                    boolean x = TaiKhoanController.Them(txtTenTaiKhoan.getText(),"1",layMaNV(cbbNhanVien.getSelectedItem().toString()),layIdQuyen(cbbQuyen.getSelectedItem().toString()));
                    if(x == false){
                ...
                if(TaiKhoanController.kiemtraDB(txtTenTaiKhoan.getText()))
                {
                    boolean x =TaiKhoanController.Sua(txtTenTaiKhoan.getText(),layMaNV(cbbNhanVien.getSelectedItem().toString()),layIdQuyen(cbbQuyen.getSelectedItem().toString()));
                    if(x == false){
    ...
    private void btLamMoiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btLamMoiActionPerformed
        if(TenTK.equals("")){
            ...
            boolean x =TaiKhoanController.SuaLaiTK(txtTenTaiKhoan.getText(),"1",layMaNV(cbbNhanVien.getSelectedItem().toString()),layIdQuyen(cbbQuyen.getSelectedItem().toString()));
            if(x == false){
                JOptionPane.showMessageDialog(this, "Sửa thành công");
                this.setVisible(false);
                form.load();
