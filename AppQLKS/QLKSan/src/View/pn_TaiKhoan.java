 ...
 * @author 84336
 */
public class pn_TaiKhoan extends javax.swing.JPanel {
    ...
    private void tbTaiKhoanMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbTaiKhoanMouseClicked
        ...
        if(evt.getClickCount() == 2){
            fr_TaiKhoan.TenTK = tbTaiKhoan.getModel().getValueAt(tbTaiKhoan.getSelectedRow(), 0).toString();
            fr_TaiKhoan form = new fr_TaiKhoan(this);
    ...
    public void load(){
        String nv= "";
        String quyen = "";
        ArrayList<TaiKhoan> DSTaiKhoan = TaiKhoanController.layDSTaiKhoan();
        DefaultTableModel model = (DefaultTableModel) tbTaiKhoan.getModel();
        model.setRowCount(0);
        for(TaiKhoan tk : DSTaiKhoan){
            quyen = TaiKhoanController.Quyen(tk.getQUYEN());
            nv = TaiKhoanController.LayTenNhanVien(tk.getMANV());
            Object[] objmodel = new Object[]{tk.getTENTAIKHOAN(),nv,quyen};
            model.addRow(objmodel);
        }
    }
    
    public void loadTimKiem(String TenTK){
        String nv= "";
        String quyen = "";
        ArrayList<TaiKhoan> DSTaiKhoan = TaiKhoanController.layDSTaiKhoanTheoTen(TenTK);
        DefaultTableModel model = (DefaultTableModel) tbTaiKhoan.getModel();
        model.setRowCount(0);
        for(TaiKhoan tk : DSTaiKhoan){
            quyen = TaiKhoanController.Quyen(tk.getQUYEN());
