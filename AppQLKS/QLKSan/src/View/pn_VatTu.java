 ...
 * @author 84336
 */
public class pn_VatTu extends javax.swing.JPanel {
    ...
    private void tbDichVuMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbDichVuMouseClicked
        ...
        if(evt.getClickCount() == 2){
            fr_VatTu.IdVatTu = Integer.parseInt(tbDichVu.getModel().getValueAt(tbDichVu.getSelectedRow(), 0).toString());
            fr_VatTu form = new fr_VatTu(this);
    ...
    public void load(){
        String ldv = "";
        ArrayList<VatTu> DSVatTu = VatTuController.layDSVatTu();
        DefaultTableModel model = (DefaultTableModel) tbDichVu.getModel();
        model.setRowCount(0);
        for(VatTu lp : DSVatTu){
            Object[] objmodel = new Object[]{lp.getIDVATTU(),lp.getTENVATTU(),lp.getXUATXU()};
            model.addRow(objmodel);
        }
    }
    
    public void loadTimKiem(String tenVT){
        String ldv = "";
        ArrayList<VatTu> DSVatTu = VatTuController.layDSVatTuTheoTen(tenVT);
        DefaultTableModel model = (DefaultTableModel) tbDichVu.getModel();
        model.setRowCount(0);
        for(VatTu lp : DSVatTu){
            Object[] objmodel = new Object[]{lp.getIDVATTU(),lp.getTENVATTU(),lp.getXUATXU()};
